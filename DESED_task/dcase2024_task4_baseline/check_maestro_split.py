...
# 設定ファイル読み込み
with open("confs/pretrained.yaml", "r") as f:
...
print(f"期待される分割比率: Train {config['training']['maestro_split']:.1%}, Valid {1-config['training']['maestro_split']:.1%}")
print(f"実際の分割比率: Train {len(maestro_train_df)/(len(maestro_train_df)+len(maestro_valid_df)):.1%}, Valid {len(maestro_valid_df)/(len(maestro_train_df)+len(maestro_valid_df)):.1%}")
