...
class ReliabilityVisualizer:
    ...
    def plot_confidence_histogram(
        ...
        for ax, (model_name, datasets) in zip(axes, models_data.items()):
            ...
            ax.set_title(f'{model_name} - Confidence Distribution ({pred_type})',
    ...
    def plot_per_class_reliability(
        ...
        # 平均ECEが高い上位K個のクラスを選択
        avg_eces = {idx: np.mean(eces) if eces else 0
                   for idx, eces in class_ece_summary.items()}
        ...
        for class_idx, _ in tqdm(top_classes[:min(top_k, len([c for c, e in top_classes if e > 0]))],
            ...
            for model_name, datasets in models_data.items():
                ...
                for dataset_name, data in datasets.items():
                    if f'probs_{pred_type}' in data and 'targets' in data:
                        ...
                        targets = (data['targets'][:, class_idx] > self.config.threshold).astype(float)

                        ece, bin_centers, bin_accs, bin_confs, bin_counts = self.metrics.compute_ece(
                            probs, targets
                        )
